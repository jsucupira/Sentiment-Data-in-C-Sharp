...
namespace Core.Common.Authentication
{
    public class OAuthBase
    {
        ...
        protected string GetAuthHeader(string url, string customerKey, string customerSecret, string accessToken, string accessTokenSecret, string httpMethod)
        {
            string normalizedString;
            string normalizeUrl;
            //string timeStamp = "1405142528";
            string timeStamp = GenerateTimeStamp();
            //string nonce = "40ee7211736986be84fba60c3753ead6";
            string nonce = GenerateNonce();


            string oauthSignature = GenerateSignature(new Uri(url), customerKey, customerSecret, accessToken, accessTokenSecret, httpMethod, timeStamp, nonce, out normalizeUrl, out normalizedString);

            const string getHeaderFormat = @"{0}oauth_nonce={1}&oauth_timestamp={2}&oauth_consumer_key={3}&oauth_signature_method={4}&oauth_version={5}&oauth_token={6}&oauth_signature={7}";

            if (httpMethod == "GET")
            {
                return string.Format(getHeaderFormat,
                    "?",
            ...
            const string postHeaderFormat = "OAuth oauth_consumer_key=\"{0}\", oauth_nonce=\"{1}\", " +
                                            "oauth_signature=\"{2}\", oauth_signature_method=\"{3}\", " +
                                            "oauth_timestamp=\"{4}\", oauth_token=\"{5}\", oauth_version=\"{6}\"";

            return string.Format(postHeaderFormat,
                Uri.EscapeDataString(customerKey),
                Uri.EscapeDataString(nonce),
                Uri.EscapeDataString(oauthSignature),
                Uri.EscapeDataString(Hmacsha1SignatureType),
